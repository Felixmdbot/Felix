			...
			for (let name in global.plugins) {
				...
				if ((usedPrefix = (match[0] || '')[0])) {
					...
					if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
						this.reply(m.chat, `🐱 Your limit runs out, please buy through ${usedPrefix}buylimit, or contact the owner sazumi-bot *+6285236226786*`, m)
						continue
					}
				...
				if (chat.welcome) {
					...
					for (let user of participants) {
					  ...
					  try {
						...
						this.sendFile(id, action === 'add' ? wel : lea, 'pp.jpg', text, null, false, {
						  mentions: [user]
						});
					  }
					}
